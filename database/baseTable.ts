...
export class BaseTableManager<T> {
	...
	async getByUniqueConstraint(constraintKey: string, value: any): Promise<T | null> {
		...
		try {
			const result = await databaseManager.executeSqlAsync(query, [value]);
		...
		try {
			...
			if (this.tableStructure.name === 'UserSettings' && 'settingKey' in data) {
				...
				if (existingRecord) {
					...
					await databaseManager.executeSqlAsync(updateQuery, [...updateValues, data.settingKey]);
			...
			// Execute the query within a transaction for atomicity
			...
			const result = await databaseManager.executeSqlAsync(selectQuery, [data.uuid]);
	...
	async remove(id: any): Promise<void> {
		...
		const uuidResult = await databaseManager.executeSqlAsync(uuidQuery, [id]);
		...
		if (uuidResult.rows.length > 0) {
			...
			if (this.tableStructure.name === 'DailyNotes') {
				...
				const dateResult = await databaseManager.executeSqlAsync(dateQuery, [id]);

				if (dateResult.rows.length > 0) {
					const date = dateResult.rows.item(0).date;
				
					// Remove associated BooleanHabits
					await databaseManager.executeSqlAsync(`DELETE FROM BooleanHabits WHERE date = ?;`, [date]);
					await this.logDeletion(this.tableStructure.name, uuid);
				
					// Remove associated QuantifiableHabits
					await databaseManager.executeSqlAsync(`DELETE FROM QuantifiableHabits WHERE date = ?;`, [date]);
	...
	async createTable(): Promise<void> {
		...
		await databaseManager.executeSqlAsync(createTableQuery);
	...
	async getByDate(date: string): Promise<T[]> {
		...
		const result = await databaseManager.executeSqlAsync(query, [date]);
	...
	async getByUuid(uuid: any): Promise<T> {
		...
		const result = await databaseManager.executeSqlAsync(query, [uuid]);
	...
	async getByDateRange(startDate: string, endDate: string): Promise<T[]> {
		...
		const result = await databaseManager.executeSqlAsync(query, [startDate, endDate]);
