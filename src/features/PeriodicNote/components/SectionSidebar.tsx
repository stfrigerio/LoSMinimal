...
const SectionSidebar: React.FC<SectionSidebarProps> = ({
	...
	const sections = useMemo(() => {
		...
		const baseSections = [
			{ id: 'objectives', title: 'Objectives', icon: 'bulb' },
			{ id: 'quantifiable', title: 'Quantifiable', icon: 'stats-chart' },
			{ id: 'money', title: 'Money', icon: 'cash' },
			{ id: 'time', title: 'Time', icon: 'time' },
			{ id: 'gpt', title: 'GPT Section', icon: 'chatbubbles' },
		];
	
		if (dateState.periodType !== 'week') {
			baseSections.splice(2, 0, { id: 'boolean', title: 'Boolean', icon: 'checkmark-circle' });
		}
	
		if (['week', 'month', 'quarter'].includes(dateState.periodType)) {
			baseSections.splice(3, 0, { id: 'sleep', title: 'Sleep', icon: 'bed' });
		}
