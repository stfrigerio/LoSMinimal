...
import { NoteData } from '@/src/types/DailyNote';
...
export const usePeriodicData = (startDate: Date, endDate: Date): UsePeriodicDataReturnType => {
	if (!(startDate instanceof Date) || isNaN(startDate.getTime())) {
        console.warn('Invalid startDate provided to usePeriodicData:', startDate);
        throw new Error('Invalid startDate provided to usePeriodicData');
    }
    if (!(endDate instanceof Date) || isNaN(endDate.getTime())) {
        console.warn('Invalid endDate provided to usePeriodicData:', endDate);
        throw new Error('Invalid endDate provided to usePeriodicData');
    }
	...
	const { startDateStr, endDateStr, previousStartDateStr, previousEndDateStr } = useMemo(() => {
		...
		const validateDate = (date: Date | undefined, name: string) => {
			if (!(date instanceof Date) || isNaN(date.getTime())) {
				console.warn(`Invalid ${name} provided to date formatting:`, date);
			}
		};

		validateDate(startDate, 'startDate');
		validateDate(endDate, 'endDate');
		validateDate(previousStartDate, 'previousStartDate');
		validateDate(previousEndDate, 'previousEndDate');
		...
		try {
			...
			throw error; // Re-throw to maintain error boundary handling
		}
