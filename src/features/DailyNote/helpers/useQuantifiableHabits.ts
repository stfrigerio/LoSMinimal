...
import { UseQuantifiableHabitsType, UseQuantifiableHabitsReturnType } from '../types/QuantifiableHabits';
import { habitThresholds } from '@/src/features/DailyNote/components/QuantifiableHabits/helpers/colors';
...
export const useQuantifiableHabits: UseQuantifiableHabitsType = (data: any, date: any): UseQuantifiableHabitsReturnType => {
	...
	// Reset optimistic updates when date or data changes
	useEffect(() => {
    ...
    const handleIncrement = async (uuid: string, habitKey: string) => {
        ...
        try {
            ...
            // Revert optimistic update if the database update fails
