...
const Axes: React.FC<AxesProps> = ({ x, y, width, height, theme, viewType, data }) => {	
	...
	const aggregatedData = aggregateData(data, viewType);

    // Parse dates, ensuring non-null values
    const xAxisTicks = aggregatedData.dates.map(d => parseDate(d)).filter((d): d is Date => d !== null);

    // Group dates by week, month, or quarter
    const groupedTicks = (() => {
        switch (viewType) {
            case 'weekly':
                return xAxisTicks; // No need to group for weekly view
            case 'monthly':
                return Array.from(
                    d3.group(xAxisTicks, d => d3.timeFormat("%Y-%m")(d)),
                    ([, dates]) => dates[0]
                );
            case 'quarterly':
                return Array.from(
                    d3.group(xAxisTicks, d => `${d.getFullYear()}-Q${Math.floor(d.getMonth() / 3) + 1}`),
                    ([, dates]) => dates[0]
                );
            default:
                return xAxisTicks;
    ...
    let limitedTicks = groupedTicks.filter((_, i) => i % step === 0);
